...
section \<open>Setup Result Commands\<close>
...
text \<open>Setup and local setup commands\<close>
...
val _ =
  let
    fun setup_result finish (name, (source, pos)) =
      ...
      |> finish
    val parse =
      Parse.embedded_ml -- (\<^keyword>\<open>=\<close> |-- Parse.position Parse.embedded_ml)
  in
    ...
    Outer_Syntax.local_theory \<^command_keyword>\<open>local_setup_result\<close>
      ...
      (parse >>
        (setup_result (Local_Theory.declaration {syntax = false, pervasive = false} o K)))
  end
\<close>
